<script lang="ts">
	...
	function solanaClaimFn(container: OrderContainer) {
		return async () => {
			...
			// Extract solver bytes32 and timestamp from OutputFilled event for each output
				...
				<ChainActionRow chainLabel={getChainName(inputChain)}>
					...
					{#snippet chips()}
						{#if isSolanaChain(inputChain)}
							...
							{@const [tokenBigInt, inputAmount] = solanaOrder.inputs[0]}
							<TokenAmountChip
								amountText={formatTokenAmount(
									inputAmount,
									getCoin({
										address: `0x${tokenBigInt.toString(16).padStart(64, "0")}`,
										chainId: inputChain
									}).decimals
								)}
								symbol={getCoin({
